...
class PolicyValueNet():
    def __init__(self, board_width, board_height, model_file=None):
        ...
        # 2. Common Networks Layers convelution卷积
        ...
        # Flatten the tensor
        self.action_conv_flat = tf.reshape(self.action_conv, [-1, 4 * board_height * board_width])#不能乘4吗？
        # 3-2 Full connected layer, the output is the log probability of moves on each slot on the board
        self.action_fc = tf.layers.dense(inputs=self.action_conv_flat, units=board_height * board_width * 4, activation=tf.nn.log_softmax)
        ...
        self.mcts_probs = tf.placeholder(tf.float32, shape=[None, board_height * board_width * 4])
