...
class PolicyValueNet():
    def __init__(self, boardWidth, boardHeight, logPath='./logs', modelPath=None):
        ...
        with tf.name_scope('input_layer'):
            self.inputData = tf.placeholder(tf.float32, shape=[None, self.inputChannelSize, boardHeight, boardWidth], name='input_data')
        ...
        with tf.name_scope('conv_layer1'):
            ...
            self.conv1 = tf.layers.conv2d(inputs=self.reshapedInputData, filters=32, kernel_size=[3, 3], padding="same", activation=tf.nn.relu)
        ...
        with tf.name_scope('conv_layer4'):
            ...
            self.conv4 = tf.layers.conv2d(inputs=self.conv3, filters=self.inputChannelSize, kernel_size=[1, 1], padding="same", activation=tf.nn.relu)
            # 调整形状为 -1*4(通道数)*4*4
            self.reshapedConv4 = tf.reshape(self.conv4, [-1, self.inputChannelSize * boardHeight * boardWidth])  # 不能乘4吗？
        with tf.name_scope('dense_layer_move_probabilities'):
            # 全连接层,这里的4是棋盘每个点有4种走子方式的意思,不是通道数
            self.moveDense = tf.layers.dense(inputs=self.reshapedConv4, units=boardHeight * boardWidth * 4, activation=tf.nn.log_softmax)
        with tf.name_scope('conv_evaluation'):
            # 估值网络, -1*4*4*2
            self.evaluationConv = tf.layers.conv2d(inputs=self.conv3, filters=2, kernel_size=[1, 1], padding="same", activation=tf.nn.relu)
            # -1*2*4*4
            self.reshapedEvaluationConv = tf.reshape(self.evaluationConv, [-1, 2 * boardHeight * boardWidth])
        with tf.name_scope('dense_layer_score1'):
            # 输出尺寸 -1*64
            self.evaluationDense1 = tf.layers.dense(inputs=self.reshapedEvaluationConv, units=boardHeight * boardWidth * 4, activation=tf.nn.relu)
        with tf.name_scope('dense_layer_score2'):
        ...
        self.summaryWriter = tf.summary.FileWriter(self.logPath, self.session.graph)
        '''
    ...
    def doOneTrain(self, batchData, mctsProbability, batchScore, learningRate):
        ...
        loss, entropy, _ = self.session.run([self.loss, self.entropy, self.optimizer], feed_dict=feed_dict)
        self.doOneTrainTimes += 1
        if self.doOneTrainTimes % self.writeLogInterval == 0:
            logs = self.session.run(self.mergedSummary, feed_dict=feed_dict)
            self.summaryWriter.add_summary(logs, self.doOneTrainTimes)
        return loss, entropy
